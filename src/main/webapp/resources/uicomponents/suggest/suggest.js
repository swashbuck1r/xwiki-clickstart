...
}}else{a.Suggest=Class.create({options:{minchars:1,method:"get",varname:"input",className:"ajaxsuggest",timeout:2500,delay:500,offsety:0,shownoresults:true,noresults:"$msg.get('core.widgets.suggest.noResults')",maxheight:250,cache:false,seps:"",icon:null,resultsParameter:"results",resultId:"id",resultValue:"value",resultInfo:"info",resultIcon:"icon",resultHint:"hint",parentContainer:"body",highlight:true,fadeOnClear:true,enableHideButton:true,insertBeforeSuggestions:null,displayValue:false,displayValueText:"$msg.get('core.widgets.suggest.valuePrefix')",align:"left",unifiedLoader:false,loaderNode:null},sInput:"",nInputChars:0,aSuggestions:[],iHighlighted:null,isActive:false,initialize:function(c,d){if(!c){return false
...
break;
default:break
}},getSuggestions:function(g){g=g.strip().toLowerCase();
...
d++){if(this.aSuggestions[d].value.substr(0,g.length).toLowerCase()==g){c.push(this.aSuggestions[d])
}}this.sInput=g;
...
}var c=new Ajax.Request(d,{method:j,requestHeaders:h,onSuccess:this.setSuggestions.bindAsEventListener(this,f,g),onFailure:function(i){new b.widgets.Notification("$msg.get('core.widgets.suggest.transportError')"+i.statusText,"error",{timeout:5})
}})
...
var l=$(this.options.parentContainer).tagName.toLowerCase()=="body"?this.fld.cumulativeOffset():this.fld.positionedOffset();
var m=this.options.width?this.options.width:(this.fld.offsetWidth-2);
if(this.options.align=="left"){e.style.left=l.left+"px"
}else{if(this.options.align=="center"){e.style.left=l.left+(this.fld.getWidth()-m-2)/2+"px"
}else{e.style.left=(l.left-m+this.fld.offsetWidth-2)+"px"
}}e.style.top=(l.top+this.fld.offsetHeight+this.options.offsety)+"px";
e.style.width=m+"px";
var c=this;
...
}}else{var o=new Element("div",{"class":"results results"+d.id}),n=new Element("div",{"class":"sourceName"});
if(this.options.unifiedLoader){o.addClassName("hidden loading")
}if(typeof d.icon!="undefined"){var k=new Image();
k.onload=function(){this.sourceHeader.setStyle({backgroundImage:"url("+this.iconImage.src+")"});
this.sourceHeader.setStyle({textIndent:(this.iconImage.width+6)+"px"})
}.bind({sourceHeader:n,iconImage:k});
k.src=d.icon
}n.insert(d.name);
o.insert(n);
var f="sourceContent "+(this.options.unifiedLoader?"":"loading");
o.insert(new Element("div",{"class":f}));
if(typeof d.before!=="undefined"){this.resultContainer.insert(d.before)
}this.resultContainer.insert(o);
...
}}if(this.options.enableHideButton&&!this.container.down(".hide-button")){var h=new Element("span",{"class":"hide-button"}).update("$msg.get('core.widgets.suggest.hide')");
...
h=new Element("span",{"class":"hide-button"}).update("$msg.get('core.widgets.suggest.hide')");
h.observe("click",this.clearSuggestions.bindAsEventListener(this));
this.container.insert({bottom:new Element("div",{"class":"hide-button-wrapper"}).update(h)})
}var j=this.container.fire("xwiki:suggest:containerPrepared",{container:this.container,suggest:this});
